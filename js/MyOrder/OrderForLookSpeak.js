...
var app = new Vue({
    ...
    mounted:function(){
        if(!window.sessionStorage.getItem('user_id')){ // 如果缓存localStorage中没有微信openId，则需用code去后台获取
            ...
            this.need =  this.getOrderDetail(this.info.cotent).need;
            this.tels =  this.getOrderDetail(this.info.cotent).tels,
                this.Message=this.getOrderDetail(this.info.cotent).Message,
                this.Num = this.getOrderDetail(this.info.cotent).Num
            this.id = this.info.id;
            this.time= this.getOrderDetail(this.info.cotent).time;
